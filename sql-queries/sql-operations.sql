...
where artist_id = 
	...
	inner join artist_info ai on u.artist_info_id = ai.id and ai.name = 'Artist1' 
	limit 1) 
...
select s.id, s.title, avg(r.rating) average_rating,
	case 
...
select s.id, s.title, s.duration_secs, plays_count, count(*) favourities_count
from favourite_songs fss
inner join songs s on fss.song_id = s.id
inner join users u on s.artist_id = u.id
where s.artist_id = 4
group by s.id, fss.song_id
having fss.song_id = s.id
...
SELECT s.id, s.title, ai.name AS artist_name, plays_count, AVG(r.rating) AS average_rating, 
...
SELECT g.name AS genre, s.title,
	AVG(r.rating) OVER (PARTITION BY g.name) AS average_rating
FROM songs s
...
select u.id, ai.name, sum(s.plays_count) as artist_plays_count from songs s
inner join users u on s.artist_id = u.id
inner join artist_info ai on u.artist_info_id = ai.id
...
union
select u.id, ai.name from users u
inner join artist_info ai on u.artist_info_id = ai.id 
